#!/system/bin/sh
# PrimeThermal-ArtisanROM vY.1.1
...
#  * GAMING mode — full stock clocks, 500/500 µs schedutil, package foreground
#    detection via dumpsys activity recents (~40 ms, skipped when list empty).
#  * GAMING hard abort — forced exit to ACTIVE when skin hits GAMING_HARD_ABORT
#    (default 54 °C), protecting hardware regardless of GAMING_IGNORE_THERMAL.
#  * POWERSAVE mode — caps all clusters at 60 % and sets conservative schedutil;
#    activatable via MODE=powersave in config.sh or the WebUI.
#  * Manual MODE override — MODE=auto|gaming|powersave overrides auto-detection.
#  * BURST detection — single-tick raw load ≥ 60 % triggers instant ACTIVE,
#    bypassing 3-tick rolling average and 15 s debounce.
#  * ENABLE_LOG — set false to silence all log output at shell level (zero I/O).
#  * Mode-specific HC trigger temps — TEMP_AUTO/GAMING/SAVE_TRIGGER per mode.
#  * Raised IDLE floors: 1600/1400/1200 MHz (V1.2: 1000/900/700).
#  * Raised LIGHT floors: 1950/2000/2200 MHz (V1.2: 1300/1500/1700).
#  * schedutil up_rate_limit_us = 500 µs on ALL modes (V1.2: 2000/1000/500 µs).
#  * Debounce timer via /proc/uptime — no date fork (V1.2 uses date +%s).
#  * Log rotation every 500 ticks unconditionally (V1.2: only on state change).
#  * Human-readable HH:MM:SS timestamps in log lines.
#  * is_gaming() anchored with head -1 to prevent false-positive package match.
#  * WebUI (webroot/index.html) — GAMING_PACKAGES editor + MODE/ENABLE_LOG UI.
#  * Cover image symlinked at startup for MMRL and KSU-Next module card.
#
...
echo "[INIT] PrimeThermal vY.1.1-ArtisanROM started $(date) PID=$$" > "$LOG"
echo "[INIT] MODE=${MODE} ENABLE_LOG=${ENABLE_LOG}" >> "$LOG"
...
(
  ...
  # Skipped entirely when GAMING_PACKAGES is empty or screen is off.
  ...
  apply_tweaks() {
    case "$1" in
      POCKET|IDLE)
        apply_schedutil "$POL0" 500 8000
        apply_schedutil "$POL4" 500 8000
        apply_schedutil "$POL6" 500 8000
        safe_write 100 "$SWAP"
  ...
  while true; do
    ...
    # ── BURST: single-tick raw load ≥ 60 % → instant ACTIVE ──────────────
    BURST=false
    [ "$IL" -ge 60 ] 2>/dev/null && BURST=true

    # ── Mode-specific HC trigger temperature (from V1.2) ──────────────────
    case "$MODE" in
    ...
    # ── Debounce ──────────────────────────────────────────────────────────
    # Instant transitions: any GAMING entry/exit, BURST-triggered ACTIVE.
    ...
    # IDLE    | 1600 MHz      | 1400 MHz    | 1200 MHz
    ...
    case "$ST" in
      ...
      CHARGING)
        ...
        ;;
      IDLE)
        F0=1600000; F4=1400000; F6=1200000
        ;;
      LIGHT)
